...
const AppliedJobsModal = ({ isOpen, onClose }) => {
    ...
    const getStatusInfo = (status) => {
        switch (status) {
            case 'new': return { label: 'Đang chờ', color: 'bg-yellow-100 text-yellow-700', icon: '⏳' };
            case 'reviewing': return { label: 'Đang xét', color: 'bg-blue-100 text-blue-700', icon: '👀' };
            case 'shortlisted': return { label: 'Phù hợp', color: 'bg-green-100 text-green-700', icon: '✓' };
            case 'rejected': return { label: 'Từ chối', color: 'bg-red-100 text-red-700', icon: '✕' };
            default: return { label: 'Không rõ', color: 'bg-gray-100 text-gray-700', icon: '?' };
        }
    };
        ...
        >
            <div className="bg-white rounded-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden shadow-2xl">
                ...
                <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-linear-to-r from-purple-600 to-purple-500">
                    ...
                    <button onClick={onClose} className="text-white hover:text-gray-200 transition-colors text-2xl leading-none">×</button>
                </div>
                                        ...
                                        <div className="flex gap-4">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex items-center justify-between text-xs text-gray-500">
                                                    ...
                                                    <a href={`/jobs/${job.job_id}`} className="text-purple-600 hover:text-purple-700 font-medium hover:underline">
                                                        Xem chi tiết →
                                                    </a>
                                                </div>
                                            </div>
